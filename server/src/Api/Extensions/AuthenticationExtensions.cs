...
public static class AuthenticationExtensions
{
    ...
    public static IServiceCollection AddJwtAuthentication(this IServiceCollection services)
    {
        ...
        // Bound through the options pipeline rather than read straight from
        // configuration: the key is settled in AddApplication, where an
        // environment variable or the local development fallback can replace
        // what the file says. Reading the section here meant tokens were
        // signed with one key and validated against another, and every
        // request failed with "key length is zero".
        services
            .AddOptions<JwtBearerOptions>(JwtBearerDefaults.AuthenticationScheme)
            .Configure<IOptions<TokenOptions>>((options, tokens) =>
            {
                TokenOptions tokenOptions = tokens.Value;
