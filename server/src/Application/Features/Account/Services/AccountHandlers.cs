...
public class LinkGoogleHandler : IRequestHandler<LinkGoogleDto, ProfileDto>
{
    private readonly IUserQuery _users;
    private readonly IUserCommand _command;
    private readonly IConfiguration _configuration;
    private readonly ILogger<LinkGoogleHandler> _logger;

    public LinkGoogleHandler(
        IUserQuery users,
        IUserCommand command,
        IConfiguration configuration,
        ILogger<LinkGoogleHandler> logger)
    {
        _users = users;
        _command = command;
    ...
    public async Task<ProfileDto> Handle(LinkGoogleDto request, CancellationToken ct)
    {
        User user = await _users.GetForUpdateAsync(request.UserId, ct)
            ?? throw new UnauthorizedException("This account no longer exists.");

        GoogleJsonWebSignature.Payload payload =
            await GoogleSignInHandler.VerifyAsync(request.credential, _configuration);

        // Already linked to this very account: saying so is friendlier than an
        // error about a conflict with itself.
        if (user.GoogleSubject == payload.Subject) return AccountRules.ToDto(user);

        if (user.GoogleSubject is not null)
            throw new ConflictException("This account is already linked to a Google account.");

        // The Google account may belong to a different Shifter user. Linking it
        // anyway would give two accounts one way in, and the sign-in lookup
        // would then pick whichever the index returned first.
        User? taken = await _users.GetByGoogleSubjectAsync(payload.Subject, ct);

        if (taken is not null)
            throw new ConflictException("That Google account is already used by another account.");

        user.GoogleSubject = payload.Subject;

        await _command.SaveAsync(ct);

        _logger.LogInformation("User {UserId} linked a Google account.", user.Id);
