...
public class BusinessController : ControllerBase
{
    ...
    /// <summary>When money is due, from whom, and whether it arrived in full.</summary>
    [HttpGet]
    ...
    /// <summary>Draws a line under one shortfall, or lifts the line again.</summary>
    [HttpGet]
    [Route("payouts/check")]
    ...
    /// <summary>The clean slate: every payment and every period verdict, gone.</summary>
    [HttpDelete]
    [Route("payouts")]
    ...
    /// <summary>What the work cost: the taxi home, the shoes, the shirt with the logo.</summary>
    [HttpPost]
    [Route("expenses")]
    ...
    /// <summary>Costs that come round: a travel pass, a locker, the whip-round for the staff room.</summary>
    [HttpGet]
    ...
    /// <summary>One month off, one button.</summary>
    [HttpPut]
    ...
    /// <summary>A biography made of shifts: how long, where, how many, what an hour was worth.</summary>
    [HttpGet]
    [Route("history")]
    ...
    /// <summary>The trophy shelf: every crossed goal, kept, newest first — plus the current run of closed weeks, said as a…</summary>
    [HttpGet]
    [Route("goals/history")]
    ...
    /// <summary>Upsert: there is only ever one goal per period per anchor, so a client that knows what it wants should not…</summary>
    [HttpPut]
    [Route("goals")]
    ...
    /// <summary>Archiving rather than deleting: the days a template sits on keep their history, and a mistake can be undone…</summary>
    [HttpPost]
    ...
    /// <summary>Applies one template across many dates at once, for dragging over the calendar and for generating a rota.</summary>
    [HttpPost]
    [Route("days/bulk")]
    ...
    /// <summary>Colours many days at once, each with its own value — a whole month, a week, or a pattern that alternates, in…</summary>
    [HttpPost]
    [Route("days/colour")]
    ...
    /// <summary>Upsert: the date in the route identifies the day, and the body carries its full contents, so repeating the…</summary>
    [HttpPut]
    [Route("days/{date}")]
    ...
    /// <summary>The palette for the calendar's non-working side.</summary>
    [HttpGet]
    ...
    /// <summary>Everything overlapping the range, so a fortnight of leave that began last month still reaches the calendar…</summary>
    [HttpGet]
    [Route("events")]
    ...
    /// <summary>The day here rather than in UTC.</summary>
    private static DateOnly Today => new Shifter.Application.Common.Time.AppClock().Today;

    /// <summary>Read from the token, never from the request.</summary>
...
/// <summary>Drawing a line under one shortfall.</summary>
