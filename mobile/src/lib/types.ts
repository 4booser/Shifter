    ...
    template.salary_period === 'hour'
      ...
      : template.salary_period === 'day'
        ...
        : template.salary_period === 'week'
          ? t('в неделю')
          : t('в месяц');
  // Not money(): rates can be fractional and rounding ₴85,5 to ₴86 would misquote the contract.
  const base =
...
/** Paid hours a template is worth: start to end, wrapping midnight, less the break. */
...
/** Something that takes a day without paying for it. */
...
/** What the pencil offers under "события". */
...
export interface DaySave {
  ...
  note: string | null;
  colour: string | null;
  /** The version this phone loaded, echoed back so the server can refuse a save over an edit made on another… */
  version?: number;
}
...
export const toSavePayload = (day: CalendarDayData | undefined): DaySave => ({
  shifts: (day?.shifts ?? []).map((entry) => ({
    ...
    // Carried for the same reason the positions below are: the server builds each shift row from the template…
    guests: entry.guests,
    zone: entry.zone,
  })),
  ...
  tips: day?.tips ?? null,
  // Clamped rather than carried: the phone has no field for the cash half, so lowering the total on the phone…
  tips_cash:
    day?.tips_cash == null
      ? null
...
/** Russian declines after a number, so a count and its word are one call: "1 раз", "2 раза", "5 раз". */
